...
function ChairpersonAccount() {
    ...
    const [userRank, setUnserRank] = useState('');
                    ...
                    try {
                        ...
                        await axios.get(`${backendUrl}/api/getData/${userId}`, {
                            ...
                            if (response.status === 200) {
                                ...
                                setUnserRank(response.data.results[0].rank);
    ...
    const addChairperson = async (e) => {
        ...
        const addData = { RorE, campus, college, fullname, email, password, user_id, userRank };
    ...
    // ###############################################################  UPDATE UNIT Chairperson REQUEST  ############################################################################################
    ...
    const updateChairperson = async (e) => {
        ...
        const updateRorEString = RorE;
        const updateUHA = { updateIdString, updateRorEString, updateCampusString, updateFullnameString, updateEmailString, updateCurrentEmailString, updateCollegeString, userRank };
    ...
    // ###############################################################  DELETE CHAIRPERSON ACCOUNT REQUEST  ############################################################################################
    ...
    const btnDelete = async () => {
        ...
        const requestDeleteId = { deleteIdString, deleteEmailString, userRank };
    ...
    useEffect(() => {
        const fetchUHAccount = async () => {
            try {
                const response = await axios.get(`${backendUrl}/unitHead/fetch/all-chairperson`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
    ...
    return (
        <div>
            <div className="content-wrapper" style={{ pointerEvents: deleteVisible || isLoading ? 'none' : '' }}>
                ...
                {/* Add Data */}
                <div className="modal fade" id="addChairperson" tabIndex={-1} role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" style={{ pointerEvents: isLoading ? 'none' : '', }}>
                                ...
                                <div className="modal-body">
                                    ...
                                    <div className="form-group" style={{ display: campus === "Dapitan" ? 'block' : 'none' }}>
                                        ...
                                        <select className="form-control" value={college} onChange={(e) => setCollege(e.target.value)}>
                                            <option value="select" selected disabled>Select College</option>
                                    ...
                                    <div className="form-group">
                                        <label>Password</label><button type='button' onClick={generatePass} className='generatePass'>generate password</button>
                ...
                <div className="modal fade" id="editChairperson" tabIndex={-1} role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" style={{ pointerEvents: isLoading ? 'none' : '', }}>
                    <div className="modal-dialog" role="document">
                        <div className="modal-content">
                            <div className="modal-header">
                ...
                <div className="card shadow mb-4">
                    <div className="card-header py-3">
                        <h6 className="m-0 font-weight-bold text-primary"><span style={{ fontSize: 20 }}>Chairperson Accounts </span>
                            <button type="button" className="btn btn-primary" data-toggle="modal" data-target="#addChairperson">
                                Add Chairperson
                                            ...
                                            {listChairperson.length === 0 ? (
                                                ...
                                                filteredList.map(item => (
                                                    <tr className="hover">
                                                        ...
                                                        <td>
                                                            <button style={{ backgroundColor: 'green' }} onClick={() => editBtn(item)} type="button" data-toggle="modal" data-target="#editChairperson" className="btn btn-primary edit_me">
                                                                EDIT <i className="fas fa-edit" />
                                                            </button>
                                                        </td>
                                                        <td>
                                                            <button name="delete_btn" onClick={() => deleteC(item)} className="btn btn-danger">DELETE <i className="fa fa-trash" /></button>
                                                        </td>
                                                    </tr>
                                                ))
                                            )}
