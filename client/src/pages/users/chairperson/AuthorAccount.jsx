...
function AuthorAccount() {
                    ...
                    try {
                        ...
                        await axios.get(`${backendUrl}/api/getData/${userId}`, {
                            ...
                            if (response.status === 200) {
                                ...
                                setUpdateRorE(response.data.results[0].RorE);
                                setUpdateCampus(response.data.results[0].campus);
                                setUpdateCollege(response.data.results[0].college);
    ...
    // ###############################################################  REQUEST TO GET ALL ADDED AUTHOR ACCOUNT  ############################################################################################
    const [listAuthor, setListAuthor] = useState([]);
    const [searchList, setSearchList] = useState('');
    const [testingLoading, setTestingLoading] = useState(true);

    const requestId = userData && userData.id;

    useEffect(() => {
        const fetchAllAuthorAccount = async () => {
            try {
                const response = await axios.get(`${backendUrl}/chairperson/fetch/all-author/${requestId}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
    ...
    return (
        <div>
            <div className="content-wrapper">
                ...
                {/* Edit Author Account */}
                <div className="modal fade" id="editAuthor" tabIndex={-1} role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" style={{ pointerEvents: isLoading ? 'none' : '' }}>
                                            ...
                                            {listAuthor.length === 0 ? (
                                                ...
                                                filteredList.map(item => (
                                                    <tr className="hover" key={item.id}>
                                                        ...
                                                        <td>
                                                            <button style={{ backgroundColor: 'green' }} data-toggle="modal" data-target="#editAuthor" onClick={() => editBtn(item)} className="btn btn-primary edit_me">
                                                                EDIT <i className="fas fa-edit" />
                                                            </button>
                                                        </td>
                                                        <td>
                                                            <button onClick={() => deleteA(item)} className="btn btn-danger">DELETE <i className="fa fa-trash" /></button>
                                                        </td>
                                                    </tr>
                                                ))
                                            )}
